...
dir <- df_tracking %>% 
  filter(contestantId == "9qsmopgutr7ut5g6workk8w4i") %>% 
  group_by(matchId, half, playerId) %>% 
  arrange(time) %>% 
  slice(1) %>%  
  ungroup() 
...
dir <- dir %>%  
  group_by(matchId, half, phase_id) %>%  
  summarize(dir = mean(x)) %>%  
  mutate(dir = ifelse(dir<0, 1, -1)) %>% 
  ungroup() %>% 
  dplyr::select(matchId, half, dir)
...
performance_dat <- df_tracking %>% 
  group_by(phase_id) %>%  
  filter(if (offense_defense=="offense") contestantId=="9qsmopgutr7ut5g6workk8w4i" else contestantId!="9qsmopgutr7ut5g6workk8w4i") %>%  
  ungroup() 
...
performance_dat <- performance_dat  %>% 
  group_by(matchId, half, phase_id, time, playerId) %>%  
...
performance_dat <- performance_dat %>% 
  left_join(dir, by=c("matchId", "half"))

performance_dat <- performance_dat %>% 
  group_by(matchId, half) %>% 
  mutate(xBall = xBall*dir) %>%  
  ungroup()
...
# save dataset 
write.csv(features, '../data/features_all_data.csv')

saveRDS(performance_dat, "output_data/performance_dat")
saveRDS(features, "output_data/features_final.rds")
